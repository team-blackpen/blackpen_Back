      ...
      try {
        ...
        if (postData.hashtag.length > 0) {
          for (let i in postData.hashtag) {
            const insHash = `INSERT INTO tb_post_hashtag (post_no, hashtag_title, status, view_seq, reg_dt) 
            VALUES (?, ?, 0, (SELECT IFNULL(MAX(view_seq) + 1, 0) FROM tb_post_hashtag Ph WHERE post_no = ?), ?);`;
            await connection.query(insHash, [postNo, postData.hashtag[i].hashtag_title, postNo, postData.regDt]);
          }
        }

        if (postData.post_img.length > 0) {
          for (let i in postData.hashtag) {
            const insImg = `INSERT INTO tb_post_img (post_no, img_url, status, view_seq, reg_dt) 
            VALUES (?, ?, 0, (SELECT IFNULL(MAX(view_seq) + 1, 0) FROM tb_post_img Pi WHERE post_no = ?), ?);`;
            await connection.query(insImg, [postNo, postData.post_img[i].img_url, postNo, postData.regDt]);
          }
        }

        if (postData.post_detail_img.length > 0) {
          for (let i in postData.hashtag) {
            const insDetailImg = `INSERT INTO tb_post_detail_img (post_no, img_url, status, view_seq, reg_dt) 
            VALUES (?, ?, 0, (SELECT IFNULL(MAX(view_seq) + 1, 0) FROM tb_post_detail_img Pdi WHERE post_no = ?), ?);`;
            await connection.query(insDetailImg, [postNo, postData.post_detail_img[i].img_url, postNo, postData.regDt]);
          }
        }
